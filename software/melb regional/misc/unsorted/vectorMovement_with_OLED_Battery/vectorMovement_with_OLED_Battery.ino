...
const float LOW_BATTERY_VOLTAGE = 14.7f;

// 0=normal, 1=180 degree rotation
int DISPLAY_ROTATION = 0;

Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, -1);

volatile bool shutdownActive = false;
...
float headingCorrection() {
  ...
  float correction =
    (error * HEADING_KP) + (headingIntegral * HEADING_KI) + (derivative * HEADING_KD);
...
float readBatteryVoltage() {
  float adcVoltage = analogRead(BATTERY_PIN) * (ADC_REF / 1023.0f);
...
void updateDisplay() {
  ...
  display.print("ON: ");
  display.println(formatTime((millis() - bootTimeMs) / 1000));

  display.print("IDLE: ");
  display.println(formatTime((millis() - lastActivityMs) / 1000));

  display.print("HEAD: ");
  display.println(currentYawDeg, 1);
