...
def send_ball_packet(sync_byte, detected, angle_deg, radius_px, pixel_count):
    ...
    # A short lead/hold time is added around the actual clocking: many
    # hardware SPI slave peripherals (including the Teensy's LPSPI) need
    # a few microseconds after CS asserts before the first bit is safely
    # latched, and a similar hold after the last bit before CS is released.
    cs.value(0)
    time.sleep_us(20)
    spi.write(packet)
    time.sleep_us(20)
