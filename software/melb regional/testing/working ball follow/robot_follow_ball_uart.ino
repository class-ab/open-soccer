/*
  ...
  The previous receiver in this file bit-banged SCK/MOSI with a CS line
  (SPI-style) to frame each 8-byte packet. That link has been replaced
  with a plain 3-wire hardware UART link (Teensy's Serial7 peripheral),
  ported from the verified-working standalone receiver
  ("teensy_dual_color_uart_receiver"). UART has no chip-select to frame
  a transfer the way SPI did, so the receiver instead scans the incoming
  byte stream for a valid sync byte, reads the following 7 bytes as one
  packet, and validates the checksum before accepting it - any stray or
  misaligned bytes are discarded and the parser re-syncs on the next
  valid sync byte. This version still recognizes two sync bytes
  (0xAA / 0xAB, for two trackable colors upstream) - both are accepted
  here as "a ball" since this robot only chases one target.

  Everything else (motors, IMU, display, battery, move(), chaseTick(),
  drive()) is untouched from the original merged sketch, aside from
  lowering the robot's overall max movement speed to 0.5 (see
  ROBOT_MAX_SPEED below).
*/
...
// ============================================================
//
// Replaces the previous bit-banged, CS-framed SPI-style link with a
// plain 3-wire hardware UART link, ported directly from the
// standalone, verified-working "teensy_dual_color_uart_receiver" test
// sketch.
//
...
//
// Both ends must use the same baud rate (BALL_UART_BAUD below).
//
// PACKET FORMAT (8 bytes, matches the OpenMV sender) - see the OpenMV
// script for the sending side:
...
//
// Since UART has no chip-select to frame a transfer the way SPI did,
// processBallPacket() (called every tick from systemTick(), the same
// place it was called from before) scans the incoming byte stream for a
// valid sync byte, then reads the following 7 bytes as one packet and
// validates the checksum before accepting it. Any stray/misaligned
// bytes are discarded and the parser re-syncs on the next valid sync
// byte. Once a packet is accepted, latestBallPacket / lastBallPacketMs
// are updated directly - the same globals chaseTick() already reads via
// getLatestBallData().

#define BALL_UART      Serial7   // Teensy 4.1 hardware UART (pins 28/29)
...
// getLatestBallData(). No longer needs to be volatile/noInterrupts()-
// guarded like the old ISR-driven SPI version did, since both the write
// (processBallPacket()) and the read (getLatestBallData()) now happen
// from plain main-loop code, never from an interrupt context.
BallPacket latestBallPacket = {false, 0.0f, 0.0f, 0};
unsigned long lastBallPacketMs = 0;

// Raw byte-stream reception state for the sync-scanning parser in
// processBallPacket() below.
...
// Hard ceiling on the robot's overall movement speed, as a 0.0-1.0
// fraction of full motor output. Applied inside drive() (the common
// path every move() and chaseTick() call eventually goes through), so
// it caps translation speed everywhere regardless of what maxSpeed an
// individual move() call requests. Lower this for a slower, more
// cautious robot; raise it (up to 1.0) for full speed.
const float ROBOT_MAX_SPEED = 0.5f;
...
void setup() {
  ...
  // Ball-tracking receiver (hardware UART, Serial7). Safe to bring this
  // up before the OpenMV cam powers up - it just sits idle until bytes
  // start arriving.
...
bool isBallSyncByte(uint8_t b) {
  ...
  //  return (b == BALL_SYNC_A || b == BALL_SYNC_B || b == BALL_SYNC_C);
}

// Validates the checksum + sync byte of one completed 8-byte packet and,
// if valid, updates latestBallPacket / lastBallPacketMs. Ported directly
// from the standalone UART test receiver's decodePacket(), narrowed to
// this robot's single-target BallPacket (color C is accepted at the
// framing level like A/B, but only A/B are treated as "a ball" here,
// matching the previous SPI version's behavior).
void decodeBallPacket(const uint8_t *p) {
  uint8_t sync = p[0];
...
// Drains the UART RX buffer and decodes any complete, checksum-valid
// packets it contains. Called every tick from systemTick(), so it runs
...
// and the same place the old SPI version's processBallPacket() was
// called from. Ported directly from the standalone UART test
// receiver's pollUart().
void processBallPacket() {
...
// Copies the latest ball packet out of the globals processBallPacket()
// writes into. Kept as its own function (rather than reading
// latestBallPacket directly from loop()/chaseTick()) so the call sites
// below didn't need to change when the receiver moved off an ISR - it's
// a plain copy now since there's no interrupt context to guard against.
void getLatestBallData(BallPacket &out) {
